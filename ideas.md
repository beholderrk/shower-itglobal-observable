...
# Текст
1. Всем привет. Доклад мой про такуцю штуку под общим названием ReactiveX. В общем цель этого доклада просто дать общее представление о том что это такое, т.к. тема на самом деле необъятная. Я проходил курс на udemy на 5 часов и там в общем-то только основы.
...
1. Сущности ReactiveX. ReactiveX использует следующий набор понятий.
1. Есть понятие Observable т.е. наблюдаемое, и есть понятие Observer - то есть наблюдатель. В rx наблюдатель подписывается на события наблюдаемого. Наблюдаемое вбрасывает оповещения в своих наблюдателей используя их методы для этого.
1. Про методы Observer. Observer характерен 3 методами которые схожи с итератором.
    * onNext - Observable использует этот метод чтобы передать наблюдателю следующее значение
    * onError - вызывается если в где-то в процессе передачи значениий случилась ошибка
    * onCompleted - вызывается когда источник закончил передавать значения
1. Observable - характерен тем что на него можно подписать observer. Всех обзерверов он хранит внутри себя.
    * в разных языках интрефейсы подписки погут отличаться, вот пример как это выглядит в js, т.е. можно передать либо 3 ф-ии либо объект с с соответствующим интерфейсом.
1. Есть еще понятие Subject (Как перевести на Русский не знаю). В общем это такая штука которая одновременно является и наблюдателем и наблюдаемым.
    * Т.е. его можно подписать на какие-то Observable и он будет получать значения из Observable и так как он сам является наблюдаемым эти значения будут прокидываться его наблюдателям. Так же, т.к. он является налюдателем можно вызывать его next на прямую,
    и эти значения тоже будут передаваться его наблюдателям.
...
1. Посмотрим на небольшой пример. Допустим у нас на странице есть поиск по пользователям. Т.е. когда мы что-то вводим в текстовое поле идут запросы на сервер и мы видим найденных пользователей.
    * Ф-я fromEvent создает обзервбл для событий change в поле ввода. Т.е. каждый раз когда мы что-то будем вводить в поле через наш обзервбл searchText будут пролетать объекты событий. Но чтобы отправить запрос на сервер нас не интересует весь объект события, нас интересует только введеное значение. Поэтому мы начинаем применять цепочку трансформаций над нашим потоком событий
    * оператор map поможет нам извлечь текущее значение. Если подумать то поидее нам не нужно отправлять запрос на каждый введенный символ, мы может делать это переодически скажем раз 400 мс, с этим нам поможет
    ...
    * ок нет проблем оператор switchmap нам с этим поможет. switchmap это один из операторов которые объединяют несколько источников в один поток используя свои правила. В качестве параметра у нас здесь создается другой observable (или промис) который выполняет запрос, если так получилось что запросы наложились друг на друга switchmap отменит предыдущие и исполнит последний. Т.е. такая небольшая экономия на запросах.
    * Где-то в приложении подписываемся на этот поток, вводим значения, получаем списки пользователей, и отрисовываем их.
1. Вот так примерно работа switchmap будет выглядеть во вкладке network.
...
# Кратко
1. общее представление, тема необъятная, udemy на 5 часов
1. расскажу историю, CREX. websocket, компоненты появляются, компоненты раньше соединения. до 5 секунд.
